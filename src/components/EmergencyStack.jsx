  ...
  return (
    <div ref={containerRef} className="w-full bg-black min-h-screen pt-40 pb-60 px-8 md:px-16 overflow-x-hidden">
      ...
      <div className="campaign-card-container max-w-6xl mx-auto relative group flex flex-col items-center">
        ...
        {emergencyCampaign.scribbles.map((sc, i) => (
          ...
          >
            <span className={`font-['Caveat'] text-white whitespace-nowrap
              ${sc.style === 'circle' ? 'border border-white/40 rounded-[50%] px-8 py-4 rotate-[-5deg] text-2xl' : ''}
              ${sc.style === 'underline' ? 'border-b border-white/40 px-2' : ''}
              ${sc.style === 'italic-dim' ? 'italic opacity-60 text-base font-light' : ''}
              text-base lg:text-3xl font-light opacity-70 tracking-wide
            `}>
              {sc.text}
            ...
            {(isHovered || activeVisible) ? (
              <div className="w-full h-full relative">
                <iframe
                  ...
                  className="w-full h-full pointer-events-none scale-[1.02] object-[center_40%]"
        ...
        <div className="mt-10 md:mt-16 border-l border-white/10 pl-6 md:pl-10 group-hover:border-white/30 transition-all duration-1000 max-w-4xl w-full">
           <p className="text-xs md:text-xl text-white/50 leading-relaxed font-light transition-colors duration-1000 group-hover:text-white/80">
